...
#        self.pool.add_task(lambda: self.irclibobj._handle_event(self, event))
#        self.pool.addJob(JobRequest(self.irclibobj._handle_event, self, event))

        if event.eventtype() in self.handlers:
            for fn in self.handlers[event.eventtype()]:
                fn(self, event)
#                self.pool.add_task(lambda: fn(self, event))
#                self.pool.addJob(JobRequest(fn, self, event))
#        if DEBUG:
#            print self.pool.track_threads()
